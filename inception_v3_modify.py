# 在实际中，需要修改inception_v3的代码，本代码基于torchvision的inception_v3的源代码简单修改得到
# inception_v3中有个辅助预测结构InceptionAux，直接从中间的特征层做分类，当你选择使用辅助是，模型输出有两个值(x, aux)。可以把aux当做一个正则项加入损失函数中。
...
class my_incption_v3(nn.Module):
    def __init__(self, model, num_classes=1000, aux_logits=True, transform_input=False):
        ...
        if aux_logits:
            self.AuxLogits = model.AuxLogits
    ...
    def forward(self, x):
        # 注意inception_v3做的标准化是([0.5,0.5,0.5],[0.5,0.5,0.5])
        if self.transform_input:
            x_ch0 = torch.unsqueeze(x[:, 0], 1) * (0.229 / 0.5) + (0.485 - 0.5) / 0.5
            x_ch1 = torch.unsqueeze(x[:, 1], 1) * (0.224 / 0.5) + (0.456 - 0.5) / 0.5
            x_ch2 = torch.unsqueeze(x[:, 2], 1) * (0.225 / 0.5) + (0.406 - 0.5) / 0.5
